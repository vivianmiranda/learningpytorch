...
def _analytic_R(theta_arcmin, z_eff, cosmo, cosmo_mid,
  ...
  formula lives; analytic_shape_ratio and rescale_xi are thin
  layout wrappers, and RescaledChi2 calls it on-device.
  ...
  cosmology uses its own ns; R = 1 for a row equal to cosmo_mid.

  One formula, two array libraries: pass numpy arrays (the
  analysis / plotting wrappers) or torch tensors (the training
  loop, computed on-device from the resident params). The body
  is plain arithmetic shared by both; only the log function and
  the input coercion differ, picked once up front. The numpy
  path is bit-identical to the original.

  theta_arcmin and z_eff are per-element geometry arrays that
  broadcast together to the element shape S (both (n_keep,) for
  the masked dv; (ntheta,1,1) and (1,nt,nt) for the full xi
  matrix), and cosmo is (N, n_param).

  Arguments:
    theta_arcmin = per-element angular scale(s) [arcmin].
    z_eff        = per-element effective source redshift(s),
                   e.g. min(z_i, z_j) for a tomographic pair.
    cosmo        = (N, n_param) parameter rows to rescale, a
                   numpy array or a torch tensor; a tensor's
                   dtype/device sets the output's.
    cosmo_mid    = (n_param,) reference ("mid") cosmology; R=1
                   for a row equal to it.
    names        = parameter column names (pgeom.names order),
                   used to locate As_1e9 / ns / H0 / omegam.
    u_star       = effective lens position (kernel peak) used
                   in the theta -> q map; ~0.5.
    include_amp  = if True, also multiply R by the surviving
                   geometric-amplitude factor
                   N ~ (Omega_m h^2)^ns / h (a second amplitude
                   direction beyond A_s). The standard run sets
                   it True; off by default.

  Returns:
    R = (N, *S), same library/dtype as cosmo.
  """
  # pick the array library once: torch when cosmo is a tensor
  # (the on-device path), numpy otherwise. log is the only math
  # call that differs; coerce casts the geometry arrays into
  # the same library/dtype/device as cosmo so they broadcast.
  ...
  iA, iN, iH, iO = (names.index(s) for s in
                    ("As_1e9", "ns", "H0", "omegam"))
  As   = cosmo[:, iA]
  ns   = cosmo[:, iN]
  ...
  # flat[None, :] inserts a leading size-1 axis -> (1, n_elem);
  # Gam[:, None] inserts a trailing one -> (N, 1). Dividing them
  # broadcasts into the full (N, n_elem) grid -- every cosmology's
  ...
  if include_amp:
    # surviving geometric amplitude N ~ (Om h^2)^ns / h: the
    # z_s and theta parts cancelled in the ratio, leaving a
    # per-cosmology scalar (a second amplitude direction).
...
def analytic_shape_ratio(
  ...
  divide it back out before the chi2. Thin wrapper over
  _analytic_R with the kept-element geometry; cross pairs use
  the lower source redshift, min(z_i, z_j).
  ...
  Arguments:
    ...
    zsrc_i      = (n_keep,) first source redshift of each
                  kept pair (geom.zsrc_i).
    zsrc_j      = (n_keep,) second source redshift
                  (geom.zsrc_j); pair = min(zsrc_i, zsrc_j).
    u_star      = kernel peak ~0.5.
    include_amp = forwarded to _analytic_R: also apply the
                  (Omega_m h^2)^ns / h amplitude factor.
  Returns:
    R = (N, n_keep) float64.
  """
...
def rescale_xi(
  ...
  matrix layout, for plotting/visual checks. Thin wrapper over
  _analytic_R with the full-block matrix geometry (all
  tomographic pairs and theta, unmasked). R > 0, so xi- keeps
  its sign; R = 1 for a curve equal to cosmo_mid.

  z_eff = min(z_i, z_j) for the cross pairs.
  ...
  Arguments:
    ...
    cosmo_mid   = (n_param,) reference cosmology.
    names       = parameter names (pgeom.names).
    z_src       = (ntomo,) source-bin peak redshifts, e.g.
                  geom.z_src; cross pairs use min(z_i, z_j).
    u_star      = kernel peak ~0.5.
    include_amp = forwarded to _analytic_R: also apply the
                  (Omega_m h^2)^ns / h amplitude factor.
  Returns:
